class CounterTray < Widget
  ...
  class CounterSelector < Widget
    def clipped_draw
      ResourceBundle.counter_tiles[0][0].draw(x,y,z+1)
    end
  end
  def initialize(x,y,z,width,height,window)
    ...
    ResourceBundle.counter_tiles.each_with_index do |counter_tile, index|
      @counter_selectors << CounterSelector.new(x+counter_x+10,y+counter_y+10,z,COUNTER_WIDTH+10,COUNTER_HEIGHT+10,window)
