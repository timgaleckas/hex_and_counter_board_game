class CounterTray < Widget
  ...
  class CounterSelector < Widget
    def initialize(x,y,z,width,height,window,options={})
      ...
      super(x,y,z,width,height,window,options)
    end
    def clipped_draw
  ...
  def initialize(x,y,z,width,height,window,options={})
    ...
    ResourceBundle.counter_tiles.each_with_index do |counter_tile, index|
      @counter_selectors << CounterSelector.new(x+counter_x+10,y+counter_y+10,z,COUNTER_WIDTH+10,COUNTER_HEIGHT+10,window,:counter_state => index)
