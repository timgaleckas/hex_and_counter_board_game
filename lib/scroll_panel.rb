class ScrollPanel < Widget
  ...
  def max_child_view_height; ([height]+child_views.to_a.map{|v|(v.y+v.height)-y}).max; end
  def max_child_view_width;  ([width] +child_views.to_a.map{|v|(v.x+v.width )-x}).max; end
